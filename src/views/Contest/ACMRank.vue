    ...
    <div v-if="problems && rank">
      <div v-if="!rank.error">
        <div class="card card-body">
          ...
          <div
            style="margin: auto; position: relative; height: 300px; width: 100%"
          >
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(stat, index) in rank.data.results" :key="stat.id">
                  ...
                  <td>
                    {{ stat.accepted_number }}/{{ stat.submission_number }}
                  </td>
                  <td>{{ get_remain(stat.total_time) }}</td>
                  <td
                    v-for="problem in problems.data"
                    :key="problem.id"
                    :style="acm_td(stat.submission_info[problem.id])"
                  >
                    <span v-if="stat.submission_info[problem.id]"
                      >{{
                        stat.submission_info[problem.id].is_ac == false
                          ? ""
                          : get_remain(
                              stat.submission_info[problem.id].ac_time
                            )
                      }}{{
                        stat.submission_info[problem.id].error_number == 0
                          ? ""
                          : "(-" +
                            stat.submission_info[problem.id].error_number +
                            ")"
                      }}</span
                    >
                  </td>
    ...
    this.$http
      .get(
        window.location.origin +
          "/api/contest/problem?contest_id=" +
  ...
  methods: {
    init() {
      ...
      this.$http
        .get(
          window.location.origin +
            "/api/contest_rank?offset=0&limit=30&contest_id=" +
          ...
          for (let i = 0; i < 10 && this.rank.data.results[i]; i++) {
            ...
            for (let j in this.rank.data.results[i].submission_info) {
              if (this.rank.data.results[i].submission_info[j].is_ac) {
                ranker.data.push({
                  x: this.rank.data.results[i].submission_info[j].ac_time,
                  y: counter,
                });
                counter++;
              }
