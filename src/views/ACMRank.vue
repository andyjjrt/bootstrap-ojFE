<template>
  <div class="container-fluid">
    <div class="card card-body" v-if="chart_ready">
      <div class="fs-4">ACM Rank</div>
      <div
        style="margin: 0 auto; position: relative; height: 400px; width: 100%"
      >
        ...
        <div class="table-responsive text-nowrap" v-if="rank">
          <table class="table">
            ...
            <tbody>
              <tr
                class="d-flex"
                v-for="(stat, index) in rank.data.results"
                :key="stat.id"
              >
                <th scope="row" class="d-block col">
                  {{ (page - 1) * 30 + index + 1 }}
                </th>
                <td class="d-block col">
                  <a
                    ...
                    @click="
                      $router.push({
                        name: 'User',
                        query: { username: stat.user.username },
                      })
                    "
                    >{{ stat.user.username }}</a
                  >
                </td>
                <td class="d-none d-md-block col">{{ stat.mood }}</td>
                <td class="col">{{ stat.accepted_number }}</td>
                <td class="d-none d-md-block col">
                  {{ stat.submission_number }}
                </td>
                <td class="d-none d-md-block col">
                  {{ ac_rate(stat.accepted_number, stat.submission_number) }}
                </td>
              </tr>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.$http
        .get(
          window.location.origin + "/api/user_rank?offset=0&limit=30&rule=ACM"
        )
        .then((response) => {
          ...
          for (let i = 0; i < 10 && this.rank.data.results[i]; i++) {
            this.chart_data.datasets[0].data.push(
              this.rank.data.results[i].accepted_number
            );
            this.chart_data.datasets[1].data.push(
              this.rank.data.results[i].submission_number
            );
            this.chart_data.labels.push(
              this.rank.data.results[i].user.username
            );
          }
