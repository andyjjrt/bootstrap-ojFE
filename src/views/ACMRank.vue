...
export default {
    ...
    methods:{
        init(){
            ...
            this.$http.get(window.location.origin + '/api/user_rank?offset=0&limit=30&rule=ACM').then(response => {
        ...
        load_page(page){
            ...
            this.rank = null
            this.$http.get(window.location.origin + '/api/user_rank?offset=' + offset + '&limit=30&rule=ACM').then(response => {
                this.rank = response.data
            })
        },
        ac_rate(ac,total){
