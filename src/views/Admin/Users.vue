          ...
          <div class="modal-body" v-if="userdata">
            <form class="row g-3" action="#" @submit.prevent="put_user">
              ...
              <div class="col-md-6">
                ...
                <select
                  class="form-select"
                  aria-label="Duefault select example"
                  v-model="userdata.admin_type"
                >
              ...
              <div class="col-md-6">
                <label class="form-label" v-if="userdata.admin_type == 'Admin'"
                  >Problem permission</label
                >
                <select
                  class="form-select"
                  aria-label="Duefault select example"
                  v-model="userdata.problem_permission"
                  v-if="userdata.admin_type == 'Admin'"
                >
              ...
              <div class="col-md-4">
                <div class="form-check form-switch">
                  ...
                  <input
                    ...
                    id="tfa_check"
                    type="checkbox"
                    v-model="userdata.two_factor_auth"
                    disabled
                  />
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-check form-switch">
                  <label class="form-check-label" for="api_check"
                    >Open API</label
                  >
                  <input
                    class="form-check-input"
                    id="api_check"
                    type="checkbox"
                    v-model="userdata.open_api"
                  />
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-check form-switch">
                  <label class="form-check-label" for="dis_check"
                    >Disabled</label
                  >
                  <input
                    class="form-check-input"
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            Sure to delete the user? The associated resources created by this
            user will be deleted as well, like problem, contest, announcement,
            etc.
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-danger" @click="delete_user">
              Delete
            </button>
          </div>
        </div>
      ...
      <div>
        <label class="btn btn-primary">
          <input
            style="display: none"
            type="file"
            @change="handle_user_import"
          />
          <i class="bi bi-cloud-arrow-up"></i> Choose File
        </label>
        &nbsp;
        <button
          v-if="import_data.users"
          class="btn btn-success"
          @click="import_user"
        >
          <i class="bi bi-cloud-arrow-up"></i> Import Users
        </button>
      </div>
      ...
      <div class="row g-3">
        <div class="col-sm-6">
          ...
          <input
            ...
            class="form-control"
            placeholder="Prefix"
            v-model="random_generate.prefix"
          />
        </div>
        <div class="col-sm-6">
          ...
          <input
            ...
            class="form-control"
            placeholder="Suffix"
            v-model="random_generate.suffix"
          />
        </div>
        <div class="col-sm-6">
          ...
          <input
            type="number"
            class="form-control"
            v-model="random_generate.number_from"
          />
        </div>
        <div class="col-sm-6">
          ...
          <input
            type="number"
            class="form-control"
            v-model="random_generate.number_to"
          />
        </div>
        <div class="col-sm-6">
          ...
          <input
            type="number"
            class="form-control"
            v-model="random_generate.password_length"
          />
        </div>
      </div>
      <br />
      <div>
        <button class="btn btn-primary" @click="generate_user">
          <i class="bi bi-people-fill"></i> Generate & Export
        </button>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    get_user(user) {
      this.userdata = null;
      this.$http
        .get(window.location.origin + "/api/admin/user?id=" + user.id)
        .then((response) => {
    ...
    put_user() {
      ...
      this.$http
        .put(window.location.origin + "/api/admin/user", this.userdata)
        .then((response) => {
    ...
    delete_user() {
      this.$http
        .delete(
          window.location.origin + "/api/admin/user?id=" + this.delete_id,
          {}
        )
        .then((response) => {
          if (!response.data.error) {
    ...
    import_user() {
      this.$http
        .post(window.location.origin + "/api/admin/user", this.import_data)
        .then((response) => {
          if (!response.data.error) {
            this.import_data = {};
            this.$success("Succeed");
    ...
    generate_user() {
      this.$http
        .post(
          window.location.origin + "/api/admin/generate_user",
