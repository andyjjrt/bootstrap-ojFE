<template>
  <div>
    <div
      class="modal fade"
      id="AnnounceEditModal"
      tabindex="-1"
      aria-labelledby="AnnounceEditModalLabel"
      aria-hidden="true"
      ref="announce_modal"
    >
      <div
    ...
    <div
      class="modal fade"
      id="AnnouncedeleteModal"
      tabindex="-1"
      aria-labelledby="AnnouncedeleteModalLabel"
      aria-hidden="true"
      ref="delete_modal"
    >
              ...
              <td>
                <div class="form-check form-switch">
                  <input
                    ...
                    @change="visible(announce)"
                  />
                </div>
              </td>
              <td>
                <span data-bs-toggle="modal" data-bs-target="#AnnounceEditModal"
                  ><button
                ...
                <span
                  ...
                  data-bs-target="#AnnouncedeleteModal"
                  ><button
                    class="btn btn-sm btn-danger"
                    @click="announce_1 = announce"
                    data-bs-toggle="tooltip"
                    data-bs-placement="top"
                    title="Delete"
                  >
      ...
      <div class="row">
        <div class="col-sm mb-3">
          <button
            ...
            data-bs-toggle="modal"
            data-bs-target="#AnnounceEditModal"
            @click="create_new"
          >
      ...
      this.$http
        .get(
          window.location.origin +
            this.url +
            "?id=" +
            this.$route.query.announce_id
        )
        .then((response) => {
          if (response.data.error) throw response.data.data;
          if (this.$route.params.manage_contest_id != undefined) {
            if (
              response.data.data.contest != this.$route.params.manage_contest_id
            ) {
  ...
  methods: {
    ...
    save() {
      ...
      if (this.mode == "edit" || this.mode == "vis") {
        this.$http
          .put(window.location.origin + this.url, this.announce_1)
          .then((response) => {
        ...
        this.$http
          .post(window.location.origin + this.url, this.announce_1)
          .then((response) => {
            if (!response.data.error) {
              this.AnnounceModal.toggle();
              this.$success("Succeed");
    ...
    delete_id() {
      this.$http
        .delete(window.location.origin + this.url + "?id=" + this.announce_1.id)
        .then((response) => {
          if (!response.data.error) {
