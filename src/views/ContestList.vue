<template>
  <div class="container-fluid">
    <div class="card">
      <div class="card-body">
        <div class="row">
          <div class="col-sm-8">
            <div class="d-flex justify-content-between">
              <div class="card-title fs-4">Contests</div>
              <div class="d-flex">
                <span class="px-1">
                  <div class="dropdown">
                    <button class="btn btn-light text-dark border dropdown-toggle btn-sm" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                      {{rule_type == "" ? "Rule" : rule_type}}
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                      <li><a class="dropdown-item" role="button" @click="query_rule_type('')">All</a></li>
                      <li><a class="dropdown-item" role="button" @click="query_rule_type('OI')">OI</a></li>
                      <li><a class="dropdown-item" role="button" @click="query_rule_type('ACM')">ACM</a></li>
                    </ul>
                  </div>
                </span>
                <span class="px-1">
                  <div class="dropdown">
                    <button class="btn btn-light text-dark border dropdown-toggle btn-sm" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                      {{status== "" ? "Status" : status == '1' ? 'Not Started' : status == '-1' ? 'Ended' : status == '0' ? 'Underway' : 'Null'}}
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                      <li><a class="dropdown-item" role="button" @click="query_status('')">All</a></li>
                      <li><a class="dropdown-item" role="button" @click="query_status('1')">Not Started</a></li>
                      <li><a class="dropdown-item" role="button" @click="query_status('-1')">Ended</a></li>
                      <li><a class="dropdown-item" role="button" @click="query_status('0')">Underway</a></li>
                    </ul>
                  </div>
                </span>
              </div>
            </div>
          </div>
          <div class="col-sm-4">
            <form action="#" @submit.prevent="to_page(1)" class="mb-3">
              <input class="form-control form-control-sm" type="text" placeholder="Keyword" v-model="keyword">
            </form>
          </div>
        </div>
        <div v-if="contests">
          <ul class="list-group list-group-flush">
            <div v-for="contest in contests.data.results" :key="contest.id" class="d-flex align-items-center list-group-item">
              <div class="flex-shrink-0">
                <img height="40" src="data:image/png;base64,">
              </div>
              <div role="button" class="flex-grow-1 ms-3 d-flex justify-content-between p-1" @click="$router.push({ name: 'Contest', params: { id:contest.id }})">
                <span>{{ contest.title }} <i class="bi bi-shield-lock" v-if="contest.contest_type == 'Password Protected'"></i><small class="d-none d-sm-block">{{ get_time(contest.start_time) }}~{{ get_time(contest.end_time) }} <span style="margin-left: 3px;" class="badge rounded-pill bg-info ">{{contest.rule_type}}</span></small></span>
                <div v-html="check_time(contest.start_time, contest.end_time)"></div>
              </div>
            </div>
            <div class="d-flex align-items-center list-group-item" v-if="contests.data.results.length == 0">
              <p height="40" class="fs-5">No Contests</p>
            </div>
          </ul>
        </div>
        <div class="d-flex justify-content-center" v-else>
          <div class="spinner-border" role="status">
            <span class="visually-hidden">Loading...</span>
          </div>
        </div>
      </div>
      <Pagination @nav="to_page" :total="total" :page="page" :perpage="10" :dress_class="'card-body border-top'" />
    </div>
  </div>
</template>

<script>
import Pagination from '@/components/Pagination.vue'
export default {
  name: "Home",
  data(){
    return{
      contests: null,
      page:1,
      total:0,
      keyword:"",
      rule_type:"",
      status:"",
    }
  },
  created() {
    this.init()
  },
  components:{
    Pagination
  },
  methods:{
    get_time(isoString){
      return new Date(isoString).toLocaleString();
    },
    init(){
      this.page = this.$route.query.page
      this.page == undefined ? this.page = 1 : this.page = parseInt(this.$route.query.page)

      this.keyword = this.$route.query.keyword
      this.keyword == undefined ? this.keyword = "" : this.keyword = this.$route.query.keyword

      this.rule_type = this.$route.query.rule_type
      this.rule_type == undefined ? this.rule_type = "" : this.rule_type = this.$route.query.rule_type

      this.status = this.$route.query.status
      this.status == undefined ? this.status = "" : this.status= this.$route.query.status

      let offset = (this.page-1) * 10
      this.$http.get(window.location.origin + '/api/contests?offset=' + offset + '&limit=10&keyword=' + this.keyword + '&rule_type=' + this.rule_type + '&status=' + this.status).then(response => {
        this.contests = response.data
        this.total = response.data.data.total
      });
    },
    check_time(start,end){
      let time_now = new Date()
      if(time_now < new Date(start)){
        return '<span class="badge bg-warning">Not Started</span>'; 
      }else if(time_now > new Date(end)){
        return '<span class="badge bg-danger">Ended</span>'; 
      }
        return '<span class="badge bg-success">Underway</span>'; 
    },
    to_page(page){
      let que = {}
      que.page = page
      que.status = this.status ? this.status : undefined
      que.keyword = this.keyword ? this.keyword : undefined
      que.rule_type = this.rule_type ? this.rule_type : undefined
      this.$router.push({ name: 'ContestList', query: que})
      this.init()
    },
    query_rule_type(rule_type){
      this.rule_type = rule_type
      this.to_page(1)
    },
    query_status(status){
      this.status = status
      this.to_page(1)
    }
  }
};
</script>

<style scoped>
  .md-display{
    display: none;
  }
  @media (min-width: 768px) {
    .md-display{
      display: block;
    }
  }
</style>
