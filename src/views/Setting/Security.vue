  ...
  <div>
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">Are you sure to revoke the session?</div>
          <div class="modal-footer">
            ...
            <button
              ...
              data-bs-dismiss="modal"
              @click="clear_session(selected_session)"
            >
              Revoke
            </button>
          </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            Two-factor Authentication is a powerful tool to protect your
            account, are you sure to close it?
          </div>
          <div class="modal-footer">
            <button
              type="button"
    ...
    <div class="card card-body">
      <div
        ...
        v-if="!sessions || !two_fac_auth_img"
      >
      ...
      <div class="row" v-else>
          ...
          <ul class="list-group list-group-flush py-2">
            <div v-for="session in sessions.data" :key="session.session_key">
              <div
                ...
                :style="{ 'background-color': current(session) }"
                :data-bs-toggle="session.current_session ? '' : 'modal'"
                data-bs-target="#confirm_modal"
                @click="selected_session = session"
              >
                  ...
                  <div class="d-flex flex-column">
                    <div class="d-flex flex-row">
                      <i
                        class="bi bi-laptop"
                        v-if="
                          get_UA(session.user_agent).device_type == 'desktop'
                        "
                      ></i>
                      <i class="bi bi-phone" v-else></i>
                      <span class="badge bg-light text-black ms-1">{{
                        session.ip
                      }}</span>
                    </div>
        ...
        <div class="col-xl-6">
          ...
          <template v-if="!$store.state.profile.data.user.two_factor_auth">
            <img
              v-if="two_fac_auth_img"
              :src="two_fac_auth_img"
              class="img-fluid"
            ...
            <form class="p-1" @submit.prevent="close2fa_modal.toggle()">
              <div class="mb-3">
                <input
                  ...
                  class="form-control"
                  placeholder="Two Factor Authentication code"
                  v-model="close2fa_.code"
                />
              </div>
              <button
                class="btn btn-danger"
                type="submit"
                :disabled="close2fa_.loading"
              >
                Close 2FA
              </button>
            </form>
...
export default {
  ...
  methods: {
    get_session() {
      this.$http
        .get(window.location.origin + "/api/sessions")
        .then((response) => {
    ...
    get_2fa() {
      this.$http
        .get(window.location.origin + "/api/two_factor_auth")
        .then((response) => {
    ...
    open2fa() {
      this.$http
        .post(window.location.origin + "/api/two_factor_auth", {
          code: this.open2fa_code,
        })
        .then((response) => {
          this.open2fa_code = "";
          if (!response.data.error) {
    ...
    close2fa() {
      ...
      this.$http
        .put(window.location.origin + "/api/two_factor_auth", {
          code: this.close2fa_.code,
        })
        .then((response) => {
