                ...
                <div class="dropdown">
                  ...
                  >
                    <li>
                      <a
                        class="dropdown-item"
                        @click="query_status(undefined, 'All')"
                        role="button"
                        >All</a
                      >
                    </li>
                    <li
                      v-for="stats in Object.keys($store.state.status_list)"
                      :key="stats"
                    >
                      <a
                        class="dropdown-item"
                        @click="
                          query_status(
                            stats,
                            $store.state.status_list[stats].name
                          )
                        "
                        role="button"
                        >{{ $store.state.status_list[stats].name }}</a
                      >
                    </li>
                  </ul>
                </div>
        ...
        <div v-if="status">
          <div v-if="!status.error" class="table-responsive">
            <table class="table text-nowrap">
              ...
              <tbody>
                <tr
                  v-for="stat in status.data.results"
                  :key="stat.id"
                  class="d-flex position-relative"
                  :class="{ can_see: stat.show_link }"
                >
                  <td class="col-3 d-none d-md-block">
                    {{ date_format(new Date(stat.create_time)) }}
                  </td>
                  <td class="col-2 d-none d-md-block">
                    <span
                      ...
                      @click="
                        $router.push({
                          name: 'Submission',
                          params: { submission_id: stat.id },
                        })
                      "
                    >
                      <span class="d-none d-lg-block"
                        ><span
                          :class="'badge bg-' + status_list[stat.result].type"
                          >{{ status_list[stat.result].name }}</span
                        ></span
                      >
                      <span class="d-block d-lg-none"
                        ><span
                          :class="'badge bg-' + status_list[stat.result].type"
                          >{{ status_list[stat.result].short }}</span
                        ></span
                      >
                    </span>
                    <span v-else>
                  ...
                  <td class="col-1 d-none d-lg-block">
                    <span v-if="stat.statistic_info.err_info">----</span
                    ><span v-else>{{ stat.statistic_info.time_cost }}ms</span>
                  </td>
                  <td class="col-1 d-none d-lg-block">
                    <span v-if="stat.statistic_info.err_info">----</span
                    ><span v-else
                      >{{
                        parseInt(stat.statistic_info.memory_cost / 1048576) + 1
                      }}MB</span
                    >
                  </td>
                  ...
                  <td class="col d-block d-md-none">
                    <span>{{ date_format2(new Date(stat.create_time)) }}</span>
                  </td>
                  <td class="col d-block d-md-none">
                    <span
                      v-if="stat.show_link"
                  ...
                  <template v-if="$store.state.profile">
                    <template v-if="$store.state.profile.data">
                      <div
                        class="position-absolute top-50 end-0 translate-middle-y d-none d-lg-block"
                        v-if="
                          $store.state.profile.data.user.admin_type ==
                            'Admin' ||
                          $store.state.profile.data.user.admin_type ==
                            'Super Admin'
                        "
  ...
  methods: {
    get_Status() {
      ...
      if (this.query.result) {
        ...
        this.stats_string =
          this.$store.state.status_list[this.query.result].name;
      }
      this.$http
        .get(window.location.origin + this.status_url, { params: this.query })
        .then((response) => {
    ...
    rejudge(id) {
      this.$http
        .get(window.location.origin + "/api/admin/submission/rejudge?id=" + id)
        .then(() => {
          this.get_Status();
        });
    },
