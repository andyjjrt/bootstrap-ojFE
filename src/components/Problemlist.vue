    ...
    <div>
        <div class="card">
                    ...
                    <div class="col-sm-8 d-flex justify-content-between">
                        ...
                        <div class="dropdown" v-if="$route.params.id == undefined">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton1">
                                ...
                                <li><a class="dropdown-item" role="button" @click="query_difficulty('High')">High</a></li>
                            </ul>
                        </div>
                    </div>
            ...
            <template v-if="$route.params.id == undefined && problems">
                <Pagination @nav="to_page" :total="total" :page="page" :perpage="20" :dress_class="'card-body border-top'" />
            </template>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        get_problem(){
            ...
            this.tag == undefined ? this.tag = "" : this.tag = this.$route.query.tag
            ...
            this.difficulty == undefined ? this.difficulty = "" : this.difficulty = this.$route.query.difficulty
            ...
            if(this.$route.params.id != undefined){
                ...
                tmp += 'offset=' + ((this.page-1)*20) + '&limit=20&tag=' + this.tag + '&keyword=' + this.keyword + '&difficulty=' + this.difficulty
            }
        ...
        solved(problem){
            ...
            if(this.$store.state.profile.data){
                if(this.$store.state.profile.data.acm_problems_status.problems){
                    if(this.$store.state.profile.data.acm_problems_status.problems[problem.id]){
                        if(this.$store.state.profile.data.acm_problems_status.problems[problem.id].status == 0){
                            return '<i class="bi bi-check-square text-success"></i>'
                        }else{
                            return '<i class="bi bi-x-square text-danger"></i>'
                        }
                ...
                if(this.$store.state.profile.data.oi_problems_status.problems){
                    if(this.$store.state.profile.data.oi_problems_status.problems[problem.id]){
                        if(this.$store.state.profile.data.oi_problems_status.problems[problem.id].status == 0){
                            return '<i class="bi bi-check-square text-success"></i>'
                        }else{
                            return '<i class="bi bi-x-square text-danger"></i>'
                        }
