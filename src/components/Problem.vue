        ...
        <div class="col-md-9">
          <div class="card card-body" v-katex>
            <div>
              ...
              <span class="float-end" v-if="problem.data.source">{{
                problem.data.source
              }}</span>
            </div>
            <hr />
            <h4>Description</h4>
            <v-md-editor
              :value="decodeURI(problem.data.description)"
              mode="preview"
            ></v-md-editor>
            <h4>Input</h4>
            <v-md-editor
              :value="decodeURI(problem.data.input_description)"
              mode="preview"
            ></v-md-editor>
            <h4>Output</h4>
            <v-md-editor
              :value="decodeURI(problem.data.output_description)"
              mode="preview"
            ></v-md-editor>
            <div class="row g-2 mb-3">
              <template v-for="(tests, i) in problem.data.samples">
                <div class="col-sm-6" :key="'input' + i">
                  <h4>
                    Input {{ i + 1 }}&nbsp;
                    <a role="button" @click="doCopy(tests.input)"
                ...
                <div class="col-sm-6 d-none d-sm-block" :key="'output1' + i">
                  <h4>
                    Output {{ i + 1 }}&nbsp;
                    <a role="button" @click="doCopy(tests.output)"
                      ><i class="bi bi-clipboard"></i
                    ></a>
                  </h4>
                </div>
            ...
            <v-md-editor
              v-if="problem.data.hint != ''"
              :value="decodeURI(problem.data.hint)"
              mode="preview"
            ></v-md-editor>
        ...
        <div class="col-3 md-no-display">
          <div class="list-group">
            <button
              ...
              @click="
                $router.push({
                  path: '/contest/' + $route.params.id + '/status',
                  query: { problem_id: problem.data._id },
                })
              "
              v-if="type == 'contest'"
            >
              <div class="p-1 text-nowrap">
                <i class="bi bi-list-task"></i> Submissions
              </div>
            </button>
            <button
              type="button"
              ...
              <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item d-flex justify-content-between">
                  ...
                  <span style="font-size: smaller"
                    >{{ problem.data.time_limit }}ms</span
                  >
                </li>
                <li class="list-group-item d-flex justify-content-between">
                  <span><i class="bi bi-hdd"></i></span>
                  <span style="font-size: smaller"
                    >{{ problem.data.memory_limit }}MB</span
                  >
                </li>
                <li class="list-group-item d-flex justify-content-between">
                  <span><i class="bi bi-code-slash"></i></span>
                  <span style="font-size: smaller">{{
                    problem.data.io_mode.io_mode
                  }}</span>
                </li>
                <li class="list-group-item d-flex justify-content-between">
                ...
                <li class="list-group-item">
                  ...
                  <div class="d-flex flex-wrap">
                    <div
                      style="padding: 3px"
                      v-for="tag in problem.data.tags"
                      :key="tag"
                    >
                      <span role="button" class="badge bg-primary">{{
                        tag
                      }}</span>
                    </div>
                  </div>
          ...
          <div class="card" v-if="problem.data.submission_number != 0">
            ...
            <div class="card-body">
              <Chart
                Type="doughnut"
                :ChartData="donught_data()"
                :Options="donught_options"
              />
              <!--
...
export default {
            ...
            callbacks: {
              label: (tooltipItems) => {
                ...
                return (
                  ...
                  tooltipItems.raw +
                  "(" +
                  ((tooltipItems.raw * 100) / total).toFixed(2) +
                  "%)"
                );
              },
            },
  ...
  created() {
    ...
    this.$http
      .get(window.location.origin + this.problem_url)
      .then((response) => {
        this.problem = response.data;
        window.document.title =
          this.$store.state.site.data.website_name_shortcut +
          " | " +
          this.problem.data.title;
        /*
  ...
  methods: {
    ...
    donught_data() {
      ...
      for (let i in keys) {
        ...
        data.datasets[0].backgroundColor.push(
          this.$store.state.status_list[keys[i]].color
        );
    ...
    edit(id) {
      if (this.$route.params.id != undefined) {
        this.$router.push({
          path: "/admin/contest/" + this.$route.params.id + "?problem_id=" + id,
        });
      } else {
