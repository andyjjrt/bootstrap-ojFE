<template>
  <div>
          ...
          <div v-if="!announcements.error">
            <ul class="list-group list-group-flush">
              ...
              >
                <div
                  role="button"
                  class="flex-fill d-flex justify-content-between p-1"
                  @click="toggle_modal(a)"
                  data-bs-toggle="modal"
                  data-bs-target="#modal_announce"
                >
                  <span>{{ a.title }}</span>
                  <span class="md-no-display"
                    ><i class="bi-alarm"></i> {{ announce_time(a) }}
                    <i class="bi bi-person-fill"></i>
                    {{ a.created_by.username }}</span
                  >
                </div>
    ...
    <div
      ...
      id="modal_announce"
      tabindex="-1"
      aria-labelledby="modal_announce_label"
      aria-hidden="true"
      ref="modal_announce"
    >
      <div class="modal-dialog modal-dialog-scrollable modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="modal_announce_label">
              {{ get_title }}
            </h5>
            <button
              type="button"
  ...
  methods: {
    ...
    edit(id) {
      if (this.$route.params.id != undefined) {
        this.$router.push({
          path:
            "/admin/contest/" + this.$route.params.id + "?announce_id=" + id,
        });
      } else {
